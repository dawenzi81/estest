...
public class JavaESTest {
  ...
  private String usernames[]={"jack","mike","jason","john"};
  private String message[]={"hello","yes","no","yeah"};
  private String mobile[]={"huawei","zhongxin","xiaomi","apple","vivo","susang"};
...
/**
     ...
     * 使用es的帮助类,創建一些隨機數據
     */
    public XContentBuilder createJsonRadom() throws Exception {
        // 创建json对象, 其中一个创建json的方式
    	Random random = new Random();
        int usernameindex=random.nextInt(3);
        int messageindex=random.nextInt(3);
        int mobileindex=random.nextInt(5);
        XContentBuilder source = XContentFactory.jsonBuilder()
            .startObject()
                .field("user", usernames[usernameindex])
                .field("phone", mobile[mobileindex])
                .field("message", mobile[mobileindex])
                .field("price", random.nextInt(3000))
                .field("postDate", new Date())
...
//        // 结果获取
        ...
        System.out.println("insert:index-"+index + " ,type- " + type + ",id- " + id + ",version- " + version + ",created: " + created);
...
//        UpdateResponse response = client.update(updateRequest).get();
    ...
    public void testUpsert() throws Exception {
        ...
        long version = response.getVersion();
        boolean created = response.isCreated();
        System.out.println("updsert:index-"+index + " ,type- " + type + ",id- " + id + ",version- " + version + ",created: " + created);
    }
    ...
    public void testBulkProcessor() throws Exception {
        ...
        for(int i=1;i<100000;i++)
        {
        	...
        	int indexrandom=7;
            int typerandom=random.nextInt(8);
        	bulkProcessor.add(new IndexRequest("twitter_"+indexrandom, "log_"+typerandom, ""+(i+20)).source(createJsonRadom()));
        }
        bulkProcessor.flush();
        // 关闭
	...
	{
		// TODO Auto-generated method stub
    	JavaESTest   jest = new JavaESTest();
    	jest.before();
    	//jest.testInsert();
    	//jest.testUpsert();
    	//jest.testBulk();
    	jest.testBulkProcessor();
